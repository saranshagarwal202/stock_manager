...
class MainWindow(qtw.QMainWindow):
    ...
    def show_sale(self):
        try:
            ...
            if not self.sale_approval_list.empty:
                self.sale_approval_list.drop(
                    "ASKING RATE", axis=1, inplace=True)
    ...
    def make_sale(self):
        try:
            ...
            if bool(self.ui.broker_name_sale.text()):
                self.ui.broker_name_sale.setPalette(
            ...
            if bool(self.ui.party_name_sale.text()):
                self.ui.party_name_sale.setPalette(
                    self.style().standardPalette())
            else:
            ...
            if passed:
                ...
                def apply_func(series):
                    ...
                    self.stock_sheet.loc[self.stock_sheet["SKU"] == series["SKU"], [
                        "Sold Qty", "Issue Qty"]] += [series["Qty Sold"], -series["Qty Sold"]]
                ...
                for i, key in enumerate(total_sale):
                    ...
                    self.ui.total_sale.setItem(i, 0, item)
    ...
    def issue_returned(self):
        ...
        try:
            ...
            if not complete_return_items.empty:
                complete_return_items.drop(
                    "complete_return", axis=1, inplace=True)
                self.issue_sheet.drop(
    ...
    def show_issue_return(self):
        try:
            ...
            self.issue_approval_list = self.issue_sheet.loc[
                self.issue_sheet["APPROVAL NO."] == approval_no]
            ...
            if bool(filepath):
                ...
                for row in range(len(self.issue_sheet_items)):
                    ...
                    try:
                        ...
                        self.issue_sheet_items.loc[row,
                                                   "SELECTION CRITERIA"] = nan
                    try:
                ...
                cwd = os.getcwd()
                doc = docx.Document(os.path.join(cwd, "approval_format.docx"))
                for p in doc.paragraphs:
                    if p.text == "# Approval No., To and Date will be printed here. Do not remove this line #":
                        p.clear()
                ...
                # clear variables
                self.clear_tabel(self.ui.issue_items,
                                 disable=True, row_count=0)
                self.sheet_index_add_sku = None
                self.issue_sheet_skus = []
    ...
    def find_sku(self):
        try:
            ...
            if not sku_found.empty:
                ...
                self.ui.Weight_to_add.setPalette(
                    self.style().standardPalette())
    ...
    def reset_selected_issue(self, event):
        try:
            ...
            if not bool(self.ui.enter_sku_code.text()):
                ...
                self.ui.enter_sku_code.setPalette(
                    self.style().standardPalette())
        except Exception:
            self._excepthook_()
    ...
    def issue_find_enter_event(self, event):
        try:
            ...
            qtw.QTableWidget.keyPressEvent(
                self.ui.issued_stock_find_entry, event)
        except Exception:
            self._excepthook_()
    ...
    def find_enter_event(self, event):
        try:
            ...
            qtw.QTableWidget.keyPressEvent(
                self.ui.current_stock_find_entry, event)
        except Exception:
            self._excepthook_()
    ...
    def reset_preview(self, tab="stock"):
        try:
            if tab == "stock":
                self.stock_sheet.loc[self.stock_sheet.index.isin(
                    self.original.index)] = self.original
                self.load_data_in_tabel(self.ui.current_stock, self.stock_sheet, len(
                    self.stock_sheet), self.schema["stock_sheet"]["columns"])
                ...
                self.issue_sheet[self.issue_sheet.index.isin(
                    self.original_issue)] = self.original_issue
    ...
    def find_item_in_stock_sheet(self, sheet):
        try:
            ...
            if sheet == "stock_sheet":
                ...
                self.original = self.stock_sheet.loc[self.stock_sheet[list(
                    search_dict)[0]].astype(str).str.contains(search_dict[list(search_dict)[0]], case=False, na=False)]
                del(search_dict[list(search_dict)[0]])
                for key in list(search_dict):
                    # if more than one search conditions
                    self.original = self.original.loc[self.original[key].str.contains(
                        search_dict[key], case=False, na=False)]
                ...
                self.original_issue = self.issue_sheet.loc[self.issue_sheet[list(
                    search_dict)[0]].astype(str).str.contains(search_dict[list(search_dict)[0]], case=False, na=False)]
                del(search_dict[list(search_dict)[0]])
                for key in list(search_dict):
                    # if more than one search conditions
                    self.original_issue = self.original_issue.loc[self.original_issue[key].str.contains(
                        search_dict[key], case=False, na=False)]
    ...
    def open_project(self, path=False):
        try:
            ...
            if bool(self.project_path):
                ...
                self.load_data_in_tabel(
                    self.ui.find_stock_preview, pd.DataFrame(columns=self.schema["stock_sheet"]["columns"]), 0, self.schema["stock_sheet"]["columns"])
                self.init_add_stock()
                ...
                self.load_data_in_tabel(
                    self.ui.issue_stock_preview, pd.DataFrame(columns=self.schema["issue_sheet"]["columns"]), 0, self.schema["issue_sheet"]["columns"])
                ...
                # issue return tab
                self.load_data_in_tabel(self.ui.issue_return_table, pd.DataFrame(
                    columns=self.schema["issue_sheet"]["columns"]), 0, self.schema["issue_sheet"]["columns"])

                # sale tab
                self.load_data_in_tabel(self.ui.sale_table, pd.DataFrame(
                    columns=self.schema["sale_sheet"]["columns"]), 0, self.schema["sale_sheet"]["columns"])
                total_sale = {
                    "Qty Sold": 0,
                    "Amount": 0,
                    "Net amount after discount": 0,
                ...
                self.ui.issue_return_table.horizontalHeader().resizeSections(
                    qtw.QHeaderView.ResizeToContents)

                self.ui.issue_return_table.horizontalHeader().resizeSections(
                    qtw.QHeaderView.ResizeToContents)

                self.ui.current_stock.horizontalHeader().setStretchLastSection(True)
                self.ui.current_entry_add_stock.horizontalHeader().setStretchLastSection(True)
                self.ui.find_stock_preview.horizontalHeader().setStretchLastSection(True)
                self.ui.current_stock_find_entry.horizontalHeader().setStretchLastSection(True)
                self.ui.issue_items.horizontalHeader().setStretchLastSection(True)
                self.ui.sku_find.horizontalHeader().setStretchLastSection(True)
                self.ui.issue_stock_preview.horizontalHeader().setStretchLastSection(True)
                self.ui.issued_stock_find_entry.horizontalHeader().setStretchLastSection(True)
                self.ui.issue_return_table.horizontalHeader().setStretchLastSection(True)
                self.ui.issue_return_table.horizontalHeader().setStretchLastSection(True)
    ...
    def New_project(self):
        try:
            ...
            if bool(self.project_path):
                cwd = os.getcwd()
                f = open(os.path.join(cwd, "schema.json"))
                self.schema = json.loads(f.read())
                f.close()
    ...
    def _excepthook_(self):
        tb = "".join(traceback.format_exc())
        msg = qtw.QMessageBox()
        msg.setIcon(qtw.QMessageBox.Critical)
        msg.setText("Critical Error Occured - contact Admin")
